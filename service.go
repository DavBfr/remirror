		...
		err := func() error {
			...
			for _, upstream := range upstreams {
				...
				if metaErr == nil {
					...
					if statErr == nil {
						...
						if mirror.should_revalidate(remote_url) {
							// Revalidate files matching action="try" patterns
							if revalidate_cache(w, r, request_path, storage_path, fileInfo, remote_url, shouldRewrite, upstreamBase, replacementURL) {
								return nil
							}
				...
				if err != nil {
					...
					if _, _, cachedStoragePath, cachedErr := get_metadata(request_path); cachedErr == nil {
						if cachedInfo, statErr := os.Stat(cachedStoragePath); statErr == nil {
							log.Printf("Remote error, serving from cache: %v", err)
							serve_cached_with_rewrite(w, r, request_path, cachedStoragePath, cachedInfo, shouldRewrite, upstreamBase, replacementURL)
							return nil
						}
					}
				...
				if shouldRewrite {
					...
					content, err := io.ReadAll(decompressedBody)
					if err != nil {
						log.Println(err)
						return nil
					}
